...
We use a separate bucket for: username/password pairs, username/active_token pairs, and for each group name. If the named group's bucket does not exist, then the group does not exist. A no-message scenario is impossible, since each POST command requires a message as a parameter.
